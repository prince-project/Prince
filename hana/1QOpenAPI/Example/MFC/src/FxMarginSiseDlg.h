...
// CFxMarginSiseDlg 대화 상자입니다.

class CFxMarginSiseDlg : public CDialog
{
public:
	CFxMarginSiseDlg(CWnd* pParent = NULL);   // 표준 생성자입니다.
...
private:
	...
	//Requet ID(조회 고유키)
	int m_nRqIdCurPrice;			//FX마진 현재가 조회 Request ID
	int m_nRqIdTick;				//FX마진 시간대별체결 조회 Request ID
	int m_nRqIdCodeList;			//FX마진 종목 리스트(마스터) 조회 Request ID
	int m_nRqIdChart;				//FX마진 차트 데이터 조회 Request ID

	int m_nSeverType;	//0-리얼, 1-국내모의, 2-해외무의

	//실시간 등록키(종목코드)
	CString m_strCurPriceRealKey;	//FX마진 현재가 실시간 등록키
	CString m_strTickRealKey;		//FX마진 시간대별체결 실시간 등록키

	//연속조회키
	CString m_strTickNextRqKey;		//FX마진 시간대별체결 연속조회키(다음 조회)
	CString m_strCodeListNextRqKey;	//FX마진 종목 리스트(마스터) 연속조회키(다음 조회)
	CString m_strChartNextRqKey;	//FX마진 차트 데이터 연속조회키(다음 조회)

private:
	//FX마진 시간대별체결 리스트 컨트롤 초기화
	void InitListTick();
...
public:
	...
	****************************************************/
	...
	CEdit m_editSymbolPrice;		//FX마진 현재가 종목코드
	CEdit m_editSymbolTick;			//FX마진 시간대별체결 종목코드
	CListCtrl m_listTick;			//FX마진 시간대별체결 리스트
	CListCtrl m_listCode;			//FX마진 종목 리스트
	...
	* FX마진 현재가 조회
	****************************************************/
	afx_msg void OnBnClickedBtnReqPrice();		//FX마진 현재가 조회

	/****************************************************
	* FX마진 시간대별체결 조회
	****************************************************/
	afx_msg void OnBnClickedBtnReqTick();		//FX마진 시간대별체결 조회
	afx_msg void OnBnClickedBtnReqTickNext();	//FX마진 시간대별체결 다음 조회

	/****************************************************
	* FX마진 차트 데이터 조회
	****************************************************/
	afx_msg void OnBnClickedBtnReqChart();		//FX마진 차트 데이터 조회
	afx_msg void OnBnClickedBtnReqChartNext();	//FX마진 차트 다음 데이터 조회
