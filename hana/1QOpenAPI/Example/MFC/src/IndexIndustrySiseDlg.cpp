...
#include "IndexIndustrySiseDlg.h"
...
// CIndexIndustrySiseDlg 대화 상자
CIndexIndustrySiseDlg::CIndexIndustrySiseDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CIndexIndustrySiseDlg::IDD, pParent)
{
	...
	m_uIdxSelectedIndustry = 0;
}

void CIndexIndustrySiseDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CIndexIndustrySiseDlg, CDialog)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_DESTROY()
	ON_WM_QUERYDRAGICON()
	...
	ON_WM_TIMER()
	ON_WM_CTLCOLOR()
	ON_BN_CLICKED(ID_LOGIN, &CIndexIndustrySiseDlg::OnBnClickedLogin)
	ON_BN_CLICKED(ID_LOGOUT, &CIndexIndustrySiseDlg::OnBnClickedLogout)
	ON_BN_CLICKED(IDC_INITCOMM, &CIndexIndustrySiseDlg::OnBnClickedInitcomm)
	ON_BN_CLICKED(IDC_TERMINATE, &CIndexIndustrySiseDlg::OnBnClickedTerminate)
	ON_BN_CLICKED(IDC_BTN_REQ_HOGA, &CIndexIndustrySiseDlg::OnBnClickedBtnReqHoga)
	ON_BN_CLICKED(IDC_BTN_REQ_PRICE, &CIndexIndustrySiseDlg::OnBnClickedBtnReqPrice)
	ON_BN_CLICKED(IDC_BTN_REQ_TICK, &CIndexIndustrySiseDlg::OnBnClickedBtnReqTick)
	ON_BN_CLICKED(IDC_BTN_REQ_TICK_NEXT, &CIndexIndustrySiseDlg::OnBnClickedBtnReqTickNext)
	ON_BN_CLICKED(IDC_BTN_REQ_CODE_LIST, &CIndexIndustrySiseDlg::OnBnClickedBtnReqCodeList)
	ON_BN_CLICKED(IDC_BTN_REQ_CODE_LIST_NEXT, &CIndexIndustrySiseDlg::OnBnClickedBtnReqCodeListNext)
	ON_NOTIFY(NM_DBLCLK, IDC_LIST_SYMBOL, &CIndexIndustrySiseDlg::OnNMDblclkListSymbol)
	ON_BN_CLICKED(IDOK, &CIndexIndustrySiseDlg::OnBnClickedOk)
	ON_BN_CLICKED(IDC_BTN_REQ_CHART, &CIndexIndustrySiseDlg::OnBnClickedBtnReqChart)
	ON_BN_CLICKED(IDC_BTN_REQ_CHART_NEXT, &CIndexIndustrySiseDlg::OnBnClickedBtnReqChartNext)
	ON_COMMAND_RANGE(IDC_RADIO_TICK, IDC_RADIO_MON, &CIndexIndustrySiseDlg::OnRadioChartTerm)
	ON_COMMAND_RANGE(IDC_CON_REAL, IDC_CON_F_DEMO, &CIndexIndustrySiseDlg::OnRdoSelectServer)
	ON_COMMAND_RANGE(IDC_RADIO_KOSPI, IDC_RADIO_KRX, &CIndexIndustrySiseDlg::OnSelectRadioIndustry)
END_MESSAGE_MAP()

// agent로부터 이벤트 수신 함수를 정의한다.
BEGIN_EVENTSINK_MAP(CIndexIndustrySiseDlg, CDialog)
	ON_EVENT(CIndexIndustrySiseDlg, IDC_HFCOMMAGENTCTRL1, 1, CIndexIndustrySiseDlg::OnGetTranData, VTS_I4 VTS_BSTR VTS_I4)
	ON_EVENT(CIndexIndustrySiseDlg, IDC_HFCOMMAGENTCTRL1, 2, CIndexIndustrySiseDlg::OnGetFidData, VTS_I4 VTS_BSTR VTS_I4)
	ON_EVENT(CIndexIndustrySiseDlg, IDC_HFCOMMAGENTCTRL1, 3, CIndexIndustrySiseDlg::OnGetRealData, VTS_BSTR VTS_BSTR VTS_BSTR VTS_I4)
END_EVENTSINK_MAP()

// CIndexIndustrySiseDlg 메시지 처리기

void CIndexIndustrySiseDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
void CIndexIndustrySiseDlg::OnPaint()
{
	if (IsIconic())
	{
...
//  이 함수를 호출합니다.
HCURSOR CIndexIndustrySiseDlg::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}

void CIndexIndustrySiseDlg::OnRdoSelectServer(UINT nID)
{
...
HBRUSH CIndexIndustrySiseDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
void CIndexIndustrySiseDlg::OnDestroy()
{
	//모든 실시간등록을 해제한다.
...
void CIndexIndustrySiseDlg::OnClose()
{
...
void CIndexIndustrySiseDlg::OnTimer(UINT_PTR nIDEvent)
{
...
void CIndexIndustrySiseDlg::OnNMDblclkListSymbol(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	*pResult = 0;
	...
	CString strSymbol = m_listCode.GetItemText(nSelectRow, 0);		//업종코드
	CString strSymbolName = m_listCode.GetItemText(nSelectRow, 1);	//업종명
	CString strPrice = m_listCode.GetItemText(nSelectRow, 2);		//지수
	...
	OnBnClickedBtnReqChart();	//지수선물 차트데이터 조회
}

void CIndexIndustrySiseDlg::OnRadioChartTerm(UINT nID)
{
	CString strCaption;
...
// Comment       : 
//******************************************************************
BOOL CIndexIndustrySiseDlg::OnInitDialog()
{
	...
	**********************************/
	...
	if (!m_CommAgent.LoadRealResource(strApidAgentModulePath + "/RealRes/U00.res"))
		::AfxMessageBox(m_CommAgent.GetLastErrMsg());

	//지수선물호가 실시간 I/O 정보 리소스 파일
	if (!m_CommAgent.LoadRealResource(strApidAgentModulePath + "/RealRes/F01.res"))
		::AfxMessageBox(m_CommAgent.GetLastErrMsg());
#endif
	...
	**********************************/
	...
	((CButton*)GetDlgItem(IDC_RADIO_KOSPI))->SetCheck(1);
...
// Comment       : 
//******************************************************************
void CIndexIndustrySiseDlg::InitListTick()
{
	CRect rc;
	...
	LV_COLUMN lvcolumn;
	TCHAR *strTitle[] = { _T("시간"), _T("지수"), _T("대비"), _T("등락률"), _T("거래량") };
...
// Comment       : 
//******************************************************************
void CIndexIndustrySiseDlg::InitListCode()
{
	CRect rc;
	...
	LV_COLUMN lvcolumn;
	TCHAR *strTitle[] = { _T("업종코드"), _T("업종명"), _T("지수") };
...
// Comment       : 
//******************************************************************
void CIndexIndustrySiseDlg::InitListChart()
{
	CRect rc;
...
// Comment       : 
//******************************************************************
CString CIndexIndustrySiseDlg::GetDiffSign(CString strSignCode)
{
...
// Comment       : 
//******************************************************************
void CIndexIndustrySiseDlg::SetHogaData(CString* arrSellHoga, CString* arrBuyHoga, CString* arrSellRemain,
...
// Comment       : 
//******************************************************************
void CIndexIndustrySiseDlg::OnBnClickedInitcomm()
{
	if (m_bCommConnect)
	{
...
// Comment       : 
//******************************************************************
void CIndexIndustrySiseDlg::OnBnClickedTerminate()
{
	if (!m_bCommConnect)
	{
...
// Comment       : 
//******************************************************************
void CIndexIndustrySiseDlg::OnBnClickedLogin()
{
	//로그인 ID
...
// Comment       : 
//******************************************************************
void CIndexIndustrySiseDlg::OnBnClickedLogout()
{
	if (!m_bCommConnect)
	{
		::AfxMessageBox("통신 접속이 끊겼습니다!");
...
// Comment       : 
//******************************************************************
void CIndexIndustrySiseDlg::OnBnClickedBtnReqCodeList()
{
	if (!m_bCommConnect)
	{
		::AfxMessageBox("통신 접속이 끊겼습니다!");
	...
	**********************************/
	...
	CString strMarketKind;		//시장구분코드

	switch (m_uIdxSelectedIndustry){
	case IDC_RADIO_KOSDAQ:
		strMarketKind = _T("Q");
		break;
	case IDC_RADIO_KOSPI200:
		strMarketKind = _T("K2");
		break;
	case IDC_RADIO_KRX:
		strMarketKind = _T("KX");
		break;
	case IDC_RADIO_KOSPI:
	default:
		strMarketKind = _T("K");
		break;
	}
	...
	* 3. 통신조회 요청 입력데이터
	**********************************/
	m_CommAgent.SetFidInputData(m_nRqIdCodeList, _T("9000"), strMarketKind);//시장구분코드
	m_CommAgent.SetFidInputData(m_nRqIdCodeList, _T("9001"), _T("U"));		//시장코드
	m_CommAgent.SetFidInputData(m_nRqIdCodeList, _T("9027"), _T("10"));		//Sort Value
	m_CommAgent.SetFidInputData(m_nRqIdCodeList, _T("9132"), _T("2"));		//입력금액 거래량 구분 코드
	m_CommAgent.SetFidInputData(m_nRqIdCodeList, _T("GID"), _T("1200"));	//GID
...
// Comment       : 
//******************************************************************
void CIndexIndustrySiseDlg::OnBnClickedBtnReqCodeListNext()
{
	if (!m_bCommConnect)
	{
		::AfxMessageBox("통신 접속이 끊겼습니다!");
...
// Comment       : 
//******************************************************************
void CIndexIndustrySiseDlg::OnBnClickedBtnReqPrice()
{
	if (!m_bCommConnect)
	{
		::AfxMessageBox("통신 접속이 끊겼습니다!");
	...
	**********************************/
	...
	{
		CString strRealName;
		strRealName = _T("U00"); //지수선물체결 Real name
	...
	**********************************/
	...
	m_CommAgent.SetFidInputData(m_nRqIdCurPrice, _T("9001"), _T("U"));	//시장구분코드
	...
	**********************************/
	...
	strOutputFidList = _T("4,6,5,7,11,28,13,14,15");
...
// Comment       : 
//******************************************************************
void CIndexIndustrySiseDlg::OnBnClickedBtnReqHoga()
{
	if (!m_bCommConnect)
	{
		::AfxMessageBox("통신 접속이 끊겼습니다!");
	...
	* 3. 통신조회 요청 입력데이터
	**********************************/
	m_CommAgent.SetFidInputData(m_nRqIdHoga, _T("9002"), strSymBol);//업종코드
	m_CommAgent.SetFidInputData(m_nRqIdHoga, _T("9001"), _T("U"));	//시장구분코드
...
// Comment       : 
//******************************************************************
void CIndexIndustrySiseDlg::OnBnClickedBtnReqTick()
{
	if (!m_bCommConnect)
	{
		::AfxMessageBox("통신 접속이 끊겼습니다!");
	...
	* 4. 통신조회 요청 입력데이터
	**********************************/
	m_CommAgent.SetFidInputData(m_nRqIdTick, _T("9002"), strSymBol);//업종코드
	m_CommAgent.SetFidInputData(m_nRqIdTick, _T("9001"), _T("U"));	//시장구분코드
	...
	**********************************/
	/* [ 통신조회 응답으로 받을 데이터 항목(FID) ]
	시간(8), 지수(4), 대비(5), 등락률(7), 거래량(11) */
	strOutputFidList = "8,4,5,7,11";
...
// Comment       : 
//******************************************************************
void CIndexIndustrySiseDlg::OnBnClickedBtnReqTickNext()
{
	if (!m_bCommConnect)
	{
		::AfxMessageBox("통신 접속이 끊겼습니다!");
...
// Function      : 지수선물 차트 데이터 조회
// Param         : 
// Return        : void 
// Create        : 
// Comment       : 
//******************************************************************
void CIndexIndustrySiseDlg::OnBnClickedBtnReqChart()
{
	if (!m_bCommConnect)
	{
		::AfxMessageBox("통신 접속이 끊겼습니다!");
	...
	**********************************/
	...
	m_CommAgent.SetFidInputData(m_nRqIdChart, _T("9001"), _T("U"));		//시장구분코드
...
// Function      : 지수선물 차트 다음 데이터 조회
// Param         : 
// Return        : void 
// Create        : 
// Comment       : 
//******************************************************************
void CIndexIndustrySiseDlg::OnBnClickedBtnReqChartNext()
{
	if (!m_bCommConnect)
	{
		::AfxMessageBox("통신 접속이 끊겼습니다!");
	...
	**********************************/
	...
	m_CommAgent.SetFidInputData(m_nRqIdChart, _T("9002"), strSymBol);	//업종코드
	m_CommAgent.SetFidInputData(m_nRqIdChart, _T("9001"), _T("U"));		//시장구분코드
	m_CommAgent.SetFidInputData(m_nRqIdChart, _T("9034"), strFromDate);	//시작일
	m_CommAgent.SetFidInputData(m_nRqIdChart, _T("9035"), strToDate);	//종료일
...
void CIndexIndustrySiseDlg::OnBnClickedOk()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
...
// Comment       : 
//******************************************************************
void CIndexIndustrySiseDlg::OnGetTranData(LONG nRequestId, LPCTSTR pBlock, long nBlockLength)
{
...
// Comment       : 
//******************************************************************
void CIndexIndustrySiseDlg::OnGetFidData(LONG nRequestId, LPCTSTR pBlock, long nBlockLength)
{
	...
	****************************************/
	if (m_nRqIdCodeList == nRequestId)
	{
		...
		for (int i = 0; i < nDataCnt; i++, nListIndex++)
		{
			...
			---------------------------------------*/
			strCode = m_CommAgent.GetFidOutputData(nRequestId, _T("1"), i);		//업종코드
			strCodeName = m_CommAgent.GetFidOutputData(nRequestId, _T("3"), i);	//업종명
			strPrice = m_CommAgent.GetFidOutputData(nRequestId, _T("4"), i);	//지수
			...
			m_listCode.SetItemText(nListIndex, 0, (LPTSTR)(LPCTSTR)strCode);		//업종코드
			m_listCode.SetItemText(nListIndex, 1, (LPTSTR)(LPCTSTR)strCodeName);	//업종명
			m_listCode.SetItemText(nListIndex, 2, (LPTSTR)(LPCTSTR)strPrice);		//지수
	...
	****************************************/
	else if (m_nRqIdTick == nRequestId)
	{
		...
		for (int i = 0; i < nDataCnt; i++, nListIndex++)
		{
			...
			strVolume = m_CommAgent.GetFidOutputData(nRequestId, "11", i);//거래량
...
// Comment       : 
//******************************************************************
void CIndexIndustrySiseDlg::OnGetRealData(LPCTSTR strRealName, LPCTSTR strRealKey, LPCTSTR pBlock, long nBlockLength)
{
	...
	****************************************/
	if (!_tcscmp(strRealName, "F01"))
	{
		...
		if (m_strHogaRealKey == strRealKey)
		{
			...
			arrSellHoga[4] = realCommRecvData.GetItem(0, 10);	//매도5호가SEQ필
	...
	****************************************/
	else if (!_tcscmp(strRealName, "U00"))
	{
		/*--------------------------------------
		아이템명으로 수신될 실시간 데이터를 항목별로 빼낸다.
		---------------------------------------*/
		CString strCurPrice = m_CommAgent.GetRealOutputData(strRealName, "PRPR_NMIX");		//지수
		CString strDiffSign = m_CommAgent.GetRealOutputData(strRealName, "PRDY_VRSS_SIGN");	//부호
		CString strDiff = m_CommAgent.GetRealOutputData(strRealName, "BSTP_NMIX_PRDY_VRSS");//전일대비
		...
		if (m_strTickRealKey == strRealKey)
		{
			...
			m_listTick.SetItemText(0, 4, (LPTSTR)(LPCTSTR)strVolume);		//거래량
...
void CIndexIndustrySiseDlg::OnSelectRadioIndustry(UINT nID)
{
	//m_uIdxSelectedIndustry = nID - IDC_RADIO_KOSPI;
	m_uIdxSelectedIndustry = nID;

	OnBnClickedBtnReqCodeList(); //업종 목록 조회
}
