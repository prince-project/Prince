...
// CIndexFutSiseDlg 대화 상자입니다.

class CIndexFutSiseDlg : public CDialog
{
public:
	CIndexFutSiseDlg(CWnd* pParent = NULL);   // 표준 생성자입니다.

// 대화 상자 데이터입니다.
	enum { IDD = IDD_INDEX_FUT_SISE_DLG };
...
private:
	...
	int m_nRqIdChart;				//지수선물 차트 데이터 조회 Request ID
	...
	CString m_strChartNextRqKey;	//지수선물 차트 데이터 연속조회키(다음 조회)
...
public:
	...
	****************************************************/
	...
	CButton m_ctrlEquityFuture;	// 주식선물용 사용 전환 체크박스
	...
	* 지수선물 차트 데이터 조회
	****************************************************/
	afx_msg void OnBnClickedBtnReqChart();		//지수선물 차트 데이터 조회
	afx_msg void OnBnClickedBtnReqChartNext();	//지수선물 차트 다음 데이터 조회
	...
	afx_msg void OnBnClickedCheckJf();
