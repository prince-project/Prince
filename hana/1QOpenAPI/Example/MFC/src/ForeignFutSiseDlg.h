...
// CForeignFutSiseDlg 대화 상자입니다.

class CForeignFutSiseDlg : public CDialog
{
public:
	CForeignFutSiseDlg(CWnd* pParent = NULL);   // 표준 생성자입니다.
...
private:
	...
	//Requet ID(조회 고유키)
	int m_nRqIdHoga;				//해외선물 5단계호가 조회 Request ID
	int m_nRqIdCurPrice;			//해외선물 현재가 조회 Request ID
	int m_nRqIdTick;				//해외선물 시간대별체결 조회 Request ID
	int m_nRqIdCodeList;			//해외선물 종목 리스트(마스터) 조회 Request ID
	int m_nRqIdCodeList2;			//해외옵션 종목 리스트(마스터) 조회 Request ID
	int m_nRqIdChart;				//해외선물 차트 데이터 조회 Request ID

	int m_nSeverType;	//0-리얼, 1-국내모의, 2-해외무의

	//실시간 등록키(종목코드)
	CString m_strHogaRealKey;		//해외선물 5단계호가 실시간 등록키
	CString m_strCurPriceRealKey;	//해외선물 현재가 실시간 등록키
	CString m_strTickRealKey;		//해외선물 시간대별체결 실시간 등록키

	//연속조회키
	CString m_strTickNextRqKey;		//해외선물 시간대별체결 연속조회키(다음 조회)
	CString m_strCodeListNextRqKey;	//해외선물 종목 리스트(마스터) 연속조회키(다음 조회)
	CString m_strCodeListNextRqKey2;//해외옵션 종목 리스트(마스터) 연속조회키(다음 조회)
	CString m_strChartNextRqKey;	//해외선물 차트 데이터 연속조회키(다음 조회)

private:
	//해외선물 시간대별체결 리스트 컨트롤 초기화
	void InitListTick();
...
public:
	...
	****************************************************/
	...
	CEdit m_editSymbolPrice;	//해외선물 현재가 종목코드
	CEdit m_editSymbol;			//해외선물 5단계호가 종목코드
	CEdit m_editSymbolTick;		//해외선물 시간대별체결 종목코드
	CListCtrl m_listTick;		//해외선물 시간대별체결 리스트
	CListCtrl m_listCode;		//해외선물 종목 리스트
	CListCtrl m_listCode2;		//해외옵션 종목 리스트
	...
	* 해외선물 현재가 조회
	****************************************************/
	afx_msg void OnBnClickedBtnReqPrice();		//해외선물 현재가 조회

	/****************************************************
	* 해외선물 5단계호가 조회
	****************************************************/
	afx_msg void OnBnClickedBtnReqHoga();		//해외선물 5단계호가 조회

	/****************************************************
	* 해외선물 시간대별체결 조회
	****************************************************/
	afx_msg void OnBnClickedBtnReqTick();		//해외선물 시간대별체결 조회
	afx_msg void OnBnClickedBtnReqTickNext();	//해외선물 시간대별체결 다음 조회

	/****************************************************
	* 해외선물 차트 데이터 조회
	****************************************************/
	afx_msg void OnBnClickedBtnReqChart();		//해외선물 차트 데이터 조회
	afx_msg void OnBnClickedBtnReqChartNext();	//해외선물 차트 다음 데이터 조회
