...
| 기      타 :
*----------------------------------------*/
// CmeFutSiseDlg.cpp : 구현 파일입니다.
//

#include "stdafx.h"
#include "HFCommClient.h"
#include "EurexFutSiseDlg.h"
...
// CEurexFutSiseDlg 대화 상자
CEurexFutSiseDlg::CEurexFutSiseDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CEurexFutSiseDlg::IDD, pParent)
{
...
void CEurexFutSiseDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CHECK_MINI, m_ctrlMiniFuture);
}

BEGIN_MESSAGE_MAP(CEurexFutSiseDlg, CDialog)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_DESTROY()
	ON_WM_QUERYDRAGICON()
	...
	ON_WM_TIMER()
	ON_WM_CTLCOLOR()
	ON_BN_CLICKED(ID_LOGIN, &CEurexFutSiseDlg::OnBnClickedLogin)
	ON_BN_CLICKED(ID_LOGOUT, &CEurexFutSiseDlg::OnBnClickedLogout)
	ON_BN_CLICKED(IDC_INITCOMM, &CEurexFutSiseDlg::OnBnClickedInitcomm)
	ON_BN_CLICKED(IDC_TERMINATE, &CEurexFutSiseDlg::OnBnClickedTerminate)
	ON_BN_CLICKED(IDC_BTN_REQ_HOGA, &CEurexFutSiseDlg::OnBnClickedBtnReqHoga)
	ON_BN_CLICKED(IDC_BTN_REQ_PRICE, &CEurexFutSiseDlg::OnBnClickedBtnReqPrice)
	ON_BN_CLICKED(IDC_BTN_REQ_TICK, &CEurexFutSiseDlg::OnBnClickedBtnReqTick)
	ON_BN_CLICKED(IDC_BTN_REQ_TICK_NEXT, &CEurexFutSiseDlg::OnBnClickedBtnReqTickNext)
	ON_BN_CLICKED(IDC_BTN_REQ_CODE_LIST, &CEurexFutSiseDlg::OnBnClickedBtnReqCodeList)
	ON_BN_CLICKED(IDC_BTN_REQ_CODE_LIST_NEXT, &CEurexFutSiseDlg::OnBnClickedBtnReqCodeListNext)
	ON_NOTIFY(NM_DBLCLK, IDC_LIST_SYMBOL, &CEurexFutSiseDlg::OnNMDblclkListSymbol)
	ON_BN_CLICKED(IDOK, &CEurexFutSiseDlg::OnBnClickedOk)
	ON_BN_CLICKED(IDC_BTN_REQ_CHART, &CEurexFutSiseDlg::OnBnClickedBtnReqChart)
	ON_BN_CLICKED(IDC_BTN_REQ_CHART_NEXT, &CEurexFutSiseDlg::OnBnClickedBtnReqChartNext)
	ON_COMMAND_RANGE(IDC_RADIO_TICK, IDC_RADIO_MON, &CEurexFutSiseDlg::OnRadioChartTerm)
	ON_COMMAND_RANGE(IDC_CON_REAL, IDC_CON_F_DEMO, &CEurexFutSiseDlg::OnRdoSelectServer)
	ON_BN_CLICKED(IDC_BUTTON_REAL, &CEurexFutSiseDlg::OnBnClickedButtonReal)
	ON_BN_CLICKED(IDC_CHECK_MINI, &CEurexFutSiseDlg::OnBnClickedCheckMini)
END_MESSAGE_MAP()

// agent로부터 이벤트 수신 함수를 정의한다.
BEGIN_EVENTSINK_MAP(CEurexFutSiseDlg, CDialog)
	ON_EVENT(CEurexFutSiseDlg, IDC_HFCOMMAGENTCTRL1, 1, CEurexFutSiseDlg::OnGetTranData, VTS_I4 VTS_BSTR VTS_I4)
	ON_EVENT(CEurexFutSiseDlg, IDC_HFCOMMAGENTCTRL1, 2, CEurexFutSiseDlg::OnGetFidData, VTS_I4 VTS_BSTR VTS_I4)
	ON_EVENT(CEurexFutSiseDlg, IDC_HFCOMMAGENTCTRL1, 3, CEurexFutSiseDlg::OnGetRealData, VTS_BSTR VTS_BSTR VTS_BSTR VTS_I4)
END_EVENTSINK_MAP()

// CEurexFutSiseDlg 메시지 처리기

void CEurexFutSiseDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
//  이 함수를 호출합니다.
HCURSOR CEurexFutSiseDlg::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}

HBRUSH CEurexFutSiseDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
// Comment       : 접속서버를 설정한다.
//******************************************************************
void CEurexFutSiseDlg::OnRdoSelectServer(UINT nID)
{
...
void CEurexFutSiseDlg::OnDestroy()
{
	//모든 실시간등록을 해제한다.
...
void CEurexFutSiseDlg::OnTimer(UINT_PTR nIDEvent)
{
...
void CEurexFutSiseDlg::OnNMDblclkListSymbol(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	*pResult = 0;
	...
	OnBnClickedBtnReqPrice();	// 현재가 조회
	OnBnClickedBtnReqHoga();	// 5단계호가 조회
	OnBnClickedBtnReqTick();	// 시간대별체결 조회
	OnBnClickedBtnReqChart();	// 차트데이터 조회
}

void CEurexFutSiseDlg::OnRadioChartTerm(UINT nID)
{
	CString strCaption;
...
// Comment       : 
//******************************************************************
BOOL CEurexFutSiseDlg::OnInitDialog()
{
	...
	CString strTitle;
	strTitle.Format("%s-[야간시장 선물(Eurex) 시세](%s)", PROGRAM_NAME, VERSION_INFO);
...
#if 0
	//체결 실시간 I/O 정보 리소스 파일
	if (!m_CommAgent.LoadRealResource(strApidAgentModulePath + "/RealRes/F40.res"))
		::AfxMessageBox(m_CommAgent.GetLastErrMsg());

	//호가 실시간 I/O 정보 리소스 파일
	if (!m_CommAgent.LoadRealResource(strApidAgentModulePath + "/RealRes/F41.res"))
		::AfxMessageBox(m_CommAgent.GetLastErrMsg());
#endif
	//차트  라디오 틱으로 선택
...
// Function      :  시간대별체결 리스트 컨트롤 초기화
// Param         : 
// Return        : void 
// Create        :
// Comment       : 
//******************************************************************
void CEurexFutSiseDlg::InitListTick()
{
	CRect rc;
...
// Function      :  종목 리스트 컨트롤 초기화
// Param         : 
// Return        : void 
// Create        : 
// Comment       : 
//******************************************************************
void CEurexFutSiseDlg::InitListCode()
{
	int arrWidth[5] = { 0x00 };
	m_listCode.SetExtendedStyle(LVS_EX_FULLROWSELECT | LVS_EX_GRIDLINES);

	LV_COLUMN lvcolumn;
	TCHAR *strTitle[] = { _T("단축코드"), _T("표준코드"), _T("    종목명    "), _T(" 기준가 "), _T("가격제한") };
...
// Comment       : 
//******************************************************************
void CEurexFutSiseDlg::InitListChart()
{
	CRect rc;
...
// Comment       : 
//******************************************************************
CString CEurexFutSiseDlg::GetDiffSign(CString strSignCode)
{
...
// Comment       : 
//******************************************************************
void CEurexFutSiseDlg::SetHogaData(CString* arrSellHoga, CString* arrBuyHoga, CString* arrSellRemain,
...
// Comment       : 
//******************************************************************
void CEurexFutSiseDlg::OnBnClickedInitcomm()
{
	if (m_bCommConnect)
	{
...
// Comment       : 
//******************************************************************
void CEurexFutSiseDlg::OnBnClickedTerminate()
{
	if (!m_bCommConnect)
	{
...
// Comment       : 
//******************************************************************
void CEurexFutSiseDlg::OnBnClickedLogin()
{
	//로그인 ID
...
// Comment       : 
//******************************************************************
void CEurexFutSiseDlg::OnBnClickedLogout()
{
	if (!m_bCommConnect)
	{
		::AfxMessageBox("통신 접속이 끊겼습니다!");
...
// Comment       : 
//******************************************************************
void CEurexFutSiseDlg::OnBnClickedBtnReqCodeList()
{
	if (!m_bCommConnect)
	{
		::AfxMessageBox("통신 접속이 끊겼습니다!");
	...
	**********************************/
	...
	if (m_ctrlMiniFuture.GetCheck())
		strMarketCode = _T("RF");	// 미니선물
...
// Function      :  종목 리스트 다음 조회
// Param         : 
// Return        : void 
// Create        : 
// Comment       : 
//******************************************************************
void CEurexFutSiseDlg::OnBnClickedBtnReqCodeListNext()
{
	if (!m_bCommConnect)
	{
		::AfxMessageBox("통신 접속이 끊겼습니다!");
...
// Comment       : 
//******************************************************************
void CEurexFutSiseDlg::OnBnClickedBtnReqPrice()
{
	if (!m_bCommConnect)
	{
		::AfxMessageBox("통신 접속이 끊겼습니다!");
	...
	**********************************/
	...
	/**********************************
	* 2. 이전에 등록된 체결(현재가) 실시간등록 해제.
	**********************************/
	if (!m_strCurPriceRealKey.IsEmpty())
	{
		CString strRealName;
		strRealName = _T("F40"); //체결 Real name
...
// Comment       : 
//******************************************************************
void CEurexFutSiseDlg::OnBnClickedBtnReqHoga()
{
	if (!m_bCommConnect)
	{
		::AfxMessageBox("통신 접속이 끊겼습니다!");
...
// Function      :  시간대별체결 데이터 조회(연속거래 데이터가 있는 경우)
// Param         : 
// Return        : void 
// Create        : 
// Comment       : 
//******************************************************************
void CEurexFutSiseDlg::OnBnClickedBtnReqTick()
{
	if (!m_bCommConnect)
	{
		::AfxMessageBox("통신 접속이 끊겼습니다!");
...
// Function      :  시간대별체결 다음 데이터 조회
// Param         : 
// Return        : void 
// Create        : 
// Comment       : 
//******************************************************************
void CEurexFutSiseDlg::OnBnClickedBtnReqTickNext()
{
	if (!m_bCommConnect)
	{
		::AfxMessageBox("통신 접속이 끊겼습니다!");
...
// Comment       : 
//******************************************************************
void CEurexFutSiseDlg::OnBnClickedBtnReqChart()
{
	if (!m_bCommConnect)
	{
		::AfxMessageBox("통신 접속이 끊겼습니다!");
...
// Function      :  차트 다음 데이터 조회
// Param         : 
// Return        : void 
// Create        : 
// Comment       : 
//******************************************************************
void CEurexFutSiseDlg::OnBnClickedBtnReqChartNext()
{
	if (!m_bCommConnect)
	{
		::AfxMessageBox("통신 접속이 끊겼습니다!");
...
void CEurexFutSiseDlg::OnBnClickedOk()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
...
// Comment       : 
//******************************************************************
void CEurexFutSiseDlg::OnGetTranData(LONG nRequestId, LPCTSTR pBlock, long nBlockLength)
{
...
// Comment       : 
//******************************************************************
void CEurexFutSiseDlg::OnGetFidData(LONG nRequestId, LPCTSTR pBlock, long nBlockLength)
{
	...
	/***************************************
	*  종목 리스트(마스터) 조회 데이터 수신 처리
	****************************************/
	if (m_nRqIdCodeList == nRequestId)
	{
		CString arrItemValue[5];
		...
		for (int i = 0; i < nDataCnt; i++, nListIndex++)
		{
			...
			arrItemValue[3] = m_CommAgent.GetFidOutputData(nRequestId, "16", i);//기준가
			arrItemValue[4] = m_CommAgent.GetFidOutputData(nRequestId, "2624", i);//실시간가격제한여부
		...
		---------------------------------------*/
		m_nRqIdCodeList = 0;	// 종목 리스트(마스터) 조회 Request ID
	}
	/***************************************
	*  5단계호가 조회 데이터 수신 처리
	****************************************/
	else if (m_nRqIdHoga == nRequestId)
	{
		...
		반드시 사용이 끝난 RQ ID는 0으로 초기화한다.
		---------------------------------------*/
		m_nRqIdHoga = 0;	// 5단계호가 조회 Request ID
	}
	/***************************************
	*  현재가 조회 데이터 수신 처리
	****************************************/
	else if (m_nRqIdCurPrice == nRequestId)
	{
		...
		반드시 사용이 끝난 RQ ID는 0으로 초기화한다.
		---------------------------------------*/
		m_nRqIdCurPrice = 0;	// 현재가 조회 Request ID
	}
	/***************************************
	*  시간대별체결 조회 데이터 수신 처리
	****************************************/
	else if (m_nRqIdTick == nRequestId)
	{
		...
		반드시 사용이 끝난 RQ ID는 0으로 초기화한다.
		---------------------------------------*/
		m_nRqIdTick = 0;	// 시간대별체결 조회 Request ID
	...
	****************************************/
	else if (m_nRqIdChart == nRequestId)
	{
		...
		****************************************/
		...
		반드시 사용이 끝난 RQ ID는 0으로 초기화한다.
		---------------------------------------*/
		m_nRqIdChart = 0;	// 종목 리스트(마스터) 조회 Request ID
...
// Comment       : 
//******************************************************************
void CEurexFutSiseDlg::OnGetRealData(LPCTSTR strRealName, LPCTSTR strRealKey, LPCTSTR pBlock, long nBlockLength)
{
	...
	*  호가 실시간 데이터 수신
	****************************************/
	if (!_tcscmp(strRealName, "F41"))
	{
		/*--------------------------------------
		조회 시점에 등록한  호가 실시간키와 동일한지 비교
		---------------------------------------*/
	...
	else if (!_tcscmp(strRealName, "FB1"))
	{
		/*--------------------------------------
		조회 시점에 등록한  호가 실시간키와 동일한지 비교
		---------------------------------------*/
	...
	*  체결(현재가) 실시간 데이터 수신
	****************************************/
	else if (!_tcscmp(strRealName, "F40"))
	{
		/*--------------------------------------
		아이템명으로 수신될 실시간 데이터를 항목별로 빼낸다.
		---------------------------------------*/
		...
		/*--------------------------------------
		조회 시점에 등록한  시간대별 체결 실시간키와 동일한지 비교
		---------------------------------------*/
	...
	else if (!_tcscmp(strRealName, "FB0"))
	{
		/*--------------------------------------
		아이템명으로 수신될 실시간 데이터를 항목별로 빼낸다.
		---------------------------------------*/
...
// Comment       : 
//******************************************************************
void CEurexFutSiseDlg::OnBnClickedButtonReal()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	if (!m_bCommConnect)
	{
	...
	if(m_CommAgent.UnRegisterReal(strRealName, strSymBol) < 0)
		::AfxMessageBox(m_CommAgent.GetLastErrMsg());
	
}

void CEurexFutSiseDlg::OnBnClickedCheckMini()
{
	OnBnClickedBtnReqCodeList(); //종목리스트(마스터) 조회
}
